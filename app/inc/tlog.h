...
#include <dirent.h>
/**********************************************************
 \brief 当多线程共享tlog对象的时候,需要使用_THREAD_SAFE编译本组件
 避免在切换文件的时候发生混乱
 ***********************************************************/
#ifdef _THREAD_SAFE
#include <pthread.h>
...
/**********************************************************
 \brief 当多进程或者多线程分别使用独立的tlog对象但是相同的配置时要使用_MP_MODE编译本组件
 避免在切换文件的时候发生混乱
 ***********************************************************/
#ifdef _MP_MODE
#endif
...
//日志类型
enum LOG_TYPE {
    LOG_TYPE_CYCLE = 0, LOG_TYPE_DAILY, LOG_TYPE_HOURLY, LOG_TYPE_CYCLE_DAILY, LOG_TYPE_CYCLE_HOURLY,
...
//日志级别
enum LOG_LEVEL {
    LOG_TRACE = 0, LOG_DEBUG, LOG_NORMAL, LOG_ERROR, LOG_FATAL, LOG_NONE //当要禁止写任何日志的时候,可以设置tlog的日志级别为LOG_NONE
};
...
#define LOG_P_ALL(lvl, fmt, args...)     log_i(LOG_FLAG_TIME | LOG_FLAG_LEVEL | LOG_FLAG_TID, lvl, "[%-10s][%d][%s]" fmt,__FILE__, __LINE__, __FUNCTION__, ##args)
//#define LOG_P_ALL(lvl, fmt, args...)     log_i(LOG_FLAG_TIME | LOG_FLAG_LEVEL | LOG_FLAG_TID, lvl, "[%d][%-10s]"fmt, __LINE__, __FUNCTION__, ##args)
//#define LOG_P_ALL(lvl, fmt, args...)    log_i(LOG_FLAG_TIME | LOG_FLAG_LEVEL | LOG_FLAG_TID, lvl, "[%-1s][%-d][%-1s]"fmt,"", "", "", ##args)
